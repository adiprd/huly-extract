...
describe("RateLimiter and TimeRateLimiter - Advanced Edge Cases", () => {
  describe("RateLimiter - Memory and Resource Management", () => {
    it("should not leak memory in notify array", async () => {
      const limiter = new import_utils.RateLimiter(1);
      const mockFn = jest.fn().mockImplementation(async () => {
        await new Promise((resolve) => setTimeout(resolve, 10));
        return "result";
      });
      const operations = Array(10).fill(0).map(() => limiter.exec(mockFn));
      await Promise.all(operations);
      expect(limiter.notify.length).toBe(0);
    });
    it("should clean up processingQueue correctly after many operations", async () => {
      const limiter = new import_utils.RateLimiter(5);
      const mockFn = jest.fn().mockImplementation(async (args) => {
        await new Promise((resolve) => setTimeout(resolve, Math.random() * 10));
        return args?.id;
      });
      for (let batch = 0; batch < 5; batch++) {
        const operations = Array(20).fill(0).map((_, i) => limiter.exec(mockFn, { id: batch * 20 + i }));
        await Promise.all(operations);
    ...
    it("should handle interleaved exec and add operations", async () => {
      const limiter = new import_utils.RateLimiter(2);
      const results = [];
      const execOp = /* @__PURE__ */ __name(async (id) => {
        await new Promise((resolve) => setTimeout(resolve, 10));
        results.push(id);
        return id;
      }, "execOp");
      const addOp = /* @__PURE__ */ __name(async (id) => {
        await new Promise((resolve) => setTimeout(resolve, 10));
        results.push(id);
      }, "addOp");
      await Promise.all([
    ...
    it("should handle rapid creation and destruction of operations", async () => {
      const limiter = new import_utils.RateLimiter(3);
      let successCount = 0;
      let errorCount = 0;
      const operations = Array(30).fill(0).map(async (_, i) => {
        const mockFn = jest.fn().mockImplementation(async () => {
  ...
  describe("TimeRateLimiter - Memory and Resource Management", () => {
    ...
    it("should not accumulate executions indefinitely", async () => {
      jest.useFakeTimers();
      const limiter = new import_utils.TimeRateLimiter(5, 1e3);
      const mockFn = jest.fn().mockResolvedValue("result");
      for (let batch = 0; batch < 10; batch++) {
        const operations = Array(5).fill(0).map(() => limiter.exec(mockFn));
        await Promise.all(operations);
  ...
  describe("TimeRateLimiter - Boundary Conditions", () => {
    it("should handle operations completing in reverse order", async () => {
      jest.useFakeTimers();
      const limiter = new import_utils.TimeRateLimiter(3, 1e3);
      const completionOrder = [];
      const createOp = /* @__PURE__ */ __name((id, delay) => async () => {
  ...
  describe("Integration Scenarios", () => {
    it("should handle mixed RateLimiter operations with varying complexities", async () => {
      const limiter = new import_utils.RateLimiter(3);
      const results = [];
      const stringOp = /* @__PURE__ */ __name(async (val) => {
        await new Promise((resolve) => setTimeout(resolve, 5));
        results.push(val);
        return val;
      }, "stringOp");
      const numberOp = /* @__PURE__ */ __name(async (val) => {
        await new Promise((resolve) => setTimeout(resolve, 10));
        results.push(val);
        return val;
      }, "numberOp");
      const objectOp = /* @__PURE__ */ __name(async (obj) => {
    ...
    it("should handle nested rate limiters", async () => {
      const outerLimiter = new import_utils.RateLimiter(2);
      const innerLimiter = new import_utils.RateLimiter(1);
      let executionCount = 0;
      const nestedOp = /* @__PURE__ */ __name(async (id) => {
...
//# sourceMappingURL=limiter-edge-cases.test.js.map
