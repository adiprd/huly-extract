...
describe("RateLimiter", () => {
  describe("constructor", () => {
    ...
    it("should handle large rates", () => {
      const limiter = new import_utils.RateLimiter(1e3);
      expect(limiter.rate).toBe(1e3);
    });
  });
  describe("exec", () => {
    ...
    it("should queue operations beyond rate limit", async () => {
      ...
      const order = [];
      const createOperation = /* @__PURE__ */ __name((id) => async () => {
        order.push(id);
        await new Promise((resolve) => setTimeout(resolve, 10));
        return id;
      }, "createOperation");
    ...
    it("should handle operations without arguments", async () => {
      ...
      expect(mockFn).toHaveBeenCalledWith(void 0);
    });
    it("should increment idCounter for each operation", async () => {
      const limiter = new import_utils.RateLimiter(10);
      const mockFn = jest.fn().mockResolvedValue("result");
    ...
    it("should notify waiting operations when slots become available", async () => {
      const limiter = new import_utils.RateLimiter(1);
      const order = [];
      const op1 = /* @__PURE__ */ __name(async () => {
        order.push("op1-start");
        await new Promise((resolve) => setTimeout(resolve, 20));
        order.push("op1-end");
        return "op1";
      }, "op1");
      const op2 = /* @__PURE__ */ __name(async () => {
  ...
  describe("add", () => {
    ...
    it("should handle errors with error handler", async () => {
      ...
      await limiter.add(mockFn, void 0, errorHandler);
      await new Promise((resolve) => setTimeout(resolve, 10));
    ...
    it("should queue multiple add operations", async () => {
      const limiter = new import_utils.RateLimiter(1);
      const order = [];
      const createOp = /* @__PURE__ */ __name((id) => async () => {
        order.push(id);
        await new Promise((resolve) => setTimeout(resolve, 10));
      }, "createOp");
  ...
  describe("edge cases", () => {
    it("should handle zero rate (should not happen but test defensively)", async () => {
      const limiter = new import_utils.RateLimiter(0);
      const mockFn = jest.fn().mockResolvedValue("result");
  ...
  describe("concurrent stress test", () => {
    it("should handle many concurrent operations correctly", async () => {
      ...
      const operations = Array(50).fill(0).map((_, i) => limiter.exec(mockFn, { id: i }));
    ...
    it("should maintain rate limit under heavy load", async () => {
      ...
      const operations = Array(20).fill(0).map(() => limiter.exec(mockFn));
      await Promise.all(operations);
