"use strict";
var import_utils = require("../rest/utils");
describe("withRetry", () => {
  it("should return result on first success", async () => {
    const fn = jest.fn().mockResolvedValue("success");
    const result = await (0, import_utils.withRetry)(fn);
    expect(result).toBe("success");
    expect(fn).toHaveBeenCalledTimes(1);
  });
  it("should retry on failure and eventually succeed", async () => {
    const fn = jest.fn().mockRejectedValueOnce(new Error("fail 1")).mockRejectedValueOnce(new Error("fail 2")).mockResolvedValue("success");
    const result = await (0, import_utils.withRetry)(fn);
    expect(result).toBe("success");
  ...
  it("should throw error after max retries", async () => {
    ...
    await expect((0, import_utils.withRetry)(fn)).rejects.toThrow("persistent failure");
    expect(fn).toHaveBeenCalledTimes(3);
  });
  it("should use exponential backoff", async () => {
    ...
    await expect((0, import_utils.withRetry)(fn)).rejects.toThrow("fail");
    expect(fn).toHaveBeenCalledTimes(3);
  ...
  it("should not decrement attempt when ignoreAttemptCheck returns true", async () => {
    ...
    const ignoreCheck = jest.fn((err) => err.message === "ignore");
    await expect((0, import_utils.withRetry)(fn, ignoreCheck)).rejects.toThrow("real error");
  ...
  it("should handle promise rejection", async () => {
    ...
    await expect((0, import_utils.withRetry)(fn)).rejects.toThrow("async error");
    expect(fn).toHaveBeenCalledTimes(3);
  });
  it("should handle errors that bypass the check", async () => {
    ...
    const ignoreCheck = jest.fn(() => false);
    await expect((0, import_utils.withRetry)(fn, ignoreCheck)).rejects.toThrow("persistent error");
    expect(fn).toHaveBeenCalledTimes(3);
...
describe("extractJson", () => {
  it("should extract plain JSON", async () => {
    ...
    const result = await (0, import_utils.extractJson)(mockResponse);
  ...
  it("should handle snappy encoding header", async () => {
    ...
    await expect((0, import_utils.extractJson)(mockResponse)).rejects.toThrow();
    expect(mockResponse.headers.get).toHaveBeenCalledWith("content-encoding");
...
//# sourceMappingURL=rest-utils.test.js.map
